...
//Map<String, String> slavedb = new HashMap<>();
	static Map<String,String> slavedb=new LRULinkedHashMap<String,String>(4);
		...
		try {
			...
			while (e1.hasNext()) {
				...
				if (tmpe.attributeValue("name").equals("kv")) {
					 ...
					 * timeout----内存淘汰
					 */
					if (slavedb.containsKey(tmpe.element("key").getText())) { // 如果内存中存在K-V键值对
						// K-V键值对没有改变
						if (slavedb.get(tmpe.element("key").getText()).equals(tmpe.element("value").getText())) {
							// 标志位不变
							live = false;
						} else {
							// 标志位置1
